export function Card(props) {
    ...
    const monthNames = ["Janeiro", "Fevereiro", "Marco", "Abril", "Maio", "Junho", "Julho", "Agosto", "Septembro", "Outubro", "Novembro", "Dezembro"];
    ...
    return (
        <div className="w-full h-20 bg-[#fffefe] shadow-md flex items-center  gap-5 relative">
            ...
            <div className="flex flex-col bg-white rounded-2 items-center font-bold shadow-sm py-2 pl-5">
                <p>{new Date(Date.parse(props.date)).getDay()}</p>
                <p>{monthNames[new Date(Date.parse(props.date)).getMonth()].substring(0, 3)}/{String(new Date(Date.parse(props.date)).getFullYear()).substring(2, 4)}</p>
            </div>
