...
export function Taxes() {
  ...
  useEffect(() => {
    let cards = [];
    axios
      .post("http://localhost:3003/contract/investment/CdbBrb", {
        address: "0xf17f52151EbEF6C7334FAD080c5704D77216b732",
      })
      .then((response) => {
        cards.push(response.data.market);
        axios
          .post("http://localhost:3003/contract/investment/Google", {
            address: "0xf17f52151EbEF6C7334FAD080c5704D77216b732",
          })
          .then((response) => {
            cards.push(response.data.market);
            axios
              .post("http://localhost:3003/contract/investment/FiiBrb", {
                address: "0xf17f52151EbEF6C7334FAD080c5704D77216b732",
              })
              .then((response) => {
                cards.push(response.data.market);
                axios
                  .post("http://localhost:3003/contract/investment/LcaBrb", {
                    address: "0xf17f52151EbEF6C7334FAD080c5704D77216b732",
                  })
                  .then((response) => {
                    cards.push(response.data.market);
                    axios
                      .post(
                        "http://localhost:3003/contract/investment/Klabin",
                        {
                          address: "0xf17f52151EbEF6C7334FAD080c5704D77216b732",
                        }
                      )
                      .then((response) => {
                        ...
                        axios
                          .post(
                            "http://localhost:3003/contract/investment/Alpargatas",
                            {
                              address:
  ...
  function makePdf() {
    ...
    let paidTable = [
      [
        ...
        { text: "Imposto de renda", style: "tableHeader" },
      ],
    ];
    let totalPaid = 0;
    cards
      ...
      .forEach((card) => {
        ...
        paidTable.push([
          ...
          "R$ " + card.value,
          "R$ " + ((card.taxes / 100) * card.value).toFixed(2),
        ]);
      });
    paidTable.push([
  ...
  const getTaxes = () => {
    ...
    investmentCards
      ...
      .filter((card) => !card.paid)
      .forEach((card) => {
        taxes += ((card.taxes / 100) * card.value).toFixed(2);
      });
    setTotalTaxes(taxes);
  };
  ...
  return (
    <div className="bg-blue h-full font-montserrat w-full flex flex-col">
      <div className="p-4 flex flex-col justify-center items-center w-full">
        <BsArrowLeft
          className="text-white text-xl absolute left-4 top-5"
          onClick={() => navigate("/")}
        />
      ...
      <div className="text-white flex w-full mt-20 justify-center">
        <button
          className={
            selectedType == "all"
              ? "bg-lightblue py-3 px-3 "
              : "bg-darkblue py-3 px-3 "
          }
        ...
        <button
          className={
            selectedType == "fixed"
              ? "bg-lightblue px-3 py-3"
              : "bg-darkblue px-3 py-3 "
          }
      ...
      <div className="bg-white text-black justify-center flex gap-1 py-5 w-full px-1 h-full">
        Total de impostos a pagar:<p className="font-bold">R$ {totalTaxes}</p>
      </div>
      ...
      <div
        ...
        className="bg-darkblue text-white px-1 py-3 w-full bottom-0 flex justify-center items-center font-bold absolute"
      >
      ...
      {selectedCard && (
        <div
          ref={ref}
          className="fixed bottom-0 h-[40%] w-full bg-white rounded-t-3xl shadow-2xl"
        >
          <div className="flex flex-col px-7 w-full">
            <span className="flex justify-center pt-4">
              ...
              <MdClose
                className="absolute right-4 top-7 text-lightblue"
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <p className="font-semibold">
                R${" "}
                {((selectedCard.taxes / 100) * selectedCard.value).toFixed(2)}
              </p>
            </div>
            ...
            <span className="flex justify-center w-full">
              <button className="bg-darkblue text-white w-fit px-4 py-2 font-bold text-lg rounded mt-8">
                Pagar
              </button>
            </span>
          </div>
      ...
      {showPaymentModal && (
        <div
          ref={payAllRef}
          className="fixed bottom-0 h-[40%] w-full bg-white rounded-t-3xl shadow-2xl overflow-scroll"
        >
          <div className="flex flex-col px-7 w-full">
            <span className="flex justify-center pt-4 ">
              <h2 className="font-bold text-black text-xl my-2">
                Detalhes da operação
              </h2>
              <MdClose
                className="absolute right-4 top-7 text-lightblue"
            ...
            <span className="flex justify-between w-full mb-5">
              <button
                className="text-gray-500 w-fit px-4 py-2 font-bold text-lg rounded mt-8"
                onClick={makePdf}
              >
                Gerar relatório
              </button>
